...
function mapClone(callback, argArr) {
  const obj = Object(this);
  const arr = new Array(obj.length);
  const len = obj.length;
  for (let k = 0; k < len; k++) {
    if (k in obj) {
      arr[k] = callback.call(argArr, obj[k], k, obj);
    }
  }
  return arr;
}
...
function asyncSum(n1, n2, callback) {
  setTimeout(() => {
...
function logNumber(err, total) {
  if (err) console.log('Error: ' + err);
  else console.log('The total is: ' + total);
}
